...
- 🤖 **AI Assistant** - Chat assistant (Perplexity) for platform questions; uses live property list when deployed on Netlify
...
- **Netlify Functions** - Optional (chat assistant using Perplexity API)
...
- **Netlify (live site)** — **Chat assistant**: In Netlify Dashboard → your site → **Site configuration** → **Environment variables**, add **`PERPLEXITY_API_KEY`** with your Perplexity API key (get it at [perplexity.ai/settings/api](https://www.perplexity.ai/settings/api)). Without this, the assistant returns "Assistant not configured".
- **Local assistant**: Add `PERPLEXITY_API_KEY` to `.env` and run `npx netlify dev` (or set `VITE_NETLIFY_URL` to your deployed URL and call the live function from local frontend).
...
- **Netlify Functions for the assistant** — Perplexity API key stays server-side; the frontend calls `/.netlify/functions/chat` so the key is never exposed in the client.
...
- Assistant chat: dynamic properties from frontend; set `PERPLEXITY_API_KEY` on Netlify for live site
